...
### Changed
- **Subtitle Wrapping Width Limits**: Reduced the subtitle wrapping width from 900px/960px to 800px to ensure subtitles do not overlap with vertical margins or social media UI overlays.

### Fixed
- **Azure TTS Timing Synchronization**: Synced the calculated subtitle timings in `compile_short` with the actual word-level timestamps in `.words.json` generated by Azure TTS, aligning narrator voice and kinetic highlighting perfectly.
- **Word Highlighting Mapping**: Added `block_start_ms` mapping to `_render_kinetic_srt_block` to correctly align the active global word timestamp to the local block word index, resolving the missing subtitle highlight bugs on subsequent frames.
- **Test Suite Verification**: Fixed pre-existing argument and assertion failures in `test_pipeline.py` to ensure 100% of unit and pipeline integration tests run clean.
